...
function makeListPage(games) {
  ...
  for (const g of games) {
    let line = `[img=60]${g.cover || g.cover2}?w=180[/img],`;
...
async function makeSalePage() {
  ...
  results = results.filter((it) => it.score > 70);
  if (skipped.length > 0) {
    console.log("makeSalePage skippped count:", skipped.length);
    for (const s of skipped) {
      console.log(`--------------------`);
      console.log(s.score, `[${s.s0}]`, s.sale.name);
      console.log(s.firstScore, `[${s.s1}]`, s.first.en_name_long);
      console.log(s.secondScore, `[${s.s2}]`, s.second.en_name_long);
      console.log(s.first.id, s.second.id);
    }
  }
  results = results.map((it) => {
    const value = {
      ...
      cover2: it.sale.cover,
      cover: it.first.img_master || it.first.img_art || it.first.img_bg,
      url: it.url,
