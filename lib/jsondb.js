...
class JsonDatabase {
  constructor(filename, debug = false) {
    ...
    this.debug = debug;
    this.debug && console.log("JsonDatabase()", this.dbFile, debug);
  }
  ...
  async _open(init = false) {
    this._log("open", this.dbFile, init);
  ...
  async bulkInsert(objArr, verbose = false) {
    ...
    try {
      ...
      db.run("ROLLBACK");
      console.error("bulkInsert", this.dbFile, error);
    }
    db.run("COMMIT");
    if (this.debug) {
      const countRet = await db.get(`SELECT count(key) from data`);
      const count = countRet["count(key)"];
      this._log("bulkInsert", this.dbFile, `Insert ${count} rows to db`);
    }
    await db.close();
    return results;
  }
  ...
  async query(key) {
    ...
    try {
      ...
      console.error("query", this.dbFile, error);
    }
    await db.close();
  }
  ...
  async queryAll(options) {
    ...
    if (this.debug) {
      const countRet = await db.get(`SELECT count(key) from data`);
      const count = countRet["count(key)"];
      console.log("queryAll", this.dbFile, `total ${count} rows in db`);
    }
    ...
    let result = await Promise.all(
      rows.map(async (row, i) => {
        try {
          ...
          console.error("queryAll", this.dbFile, error);
        }
      })
    );
    result = result.filter(options.filterFn);
    await db.close();
    this._log("queryAll", this.dbFile, `return ${result.length} rows from db`);
    return result;
  }
}
