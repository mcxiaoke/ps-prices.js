...
async function storeAll() {
  ...
  for (const p of cnp) {
    ...
    const fullNameEn = (p2 && p2.name.trim()) || fullName;
    const product = {
      id: p.id,
      tid: p.npTitleId,
      name: fullName.split("(")[0].trim(),
      name_long: fullName,
      en_name: fullNameEn.split("(")[0].trim(),
      en_name_long: fullNameEn,
      type: p.__typename,
  ...
  await fs.writeJSON("./data/store/store_games.json", products, { spaces: 2 });
  console.log(products.length);
  const db = await createDB("./data/store/store_games.db", true);
...
async function loadAll() {
  ...
  //   filterFn: (it) => it.id.includes("USA"),
  // });
  const products = await db.queryAll();
...
module.exports = { storeAll, loadAll };

// console.log("os.current.__filename=", __filename);
// console.log("require.main.filename=", require.main.filename);

if (require.main.filename === __filename) {
  console.log("module loaded directly");
  loadAll();
}
